...
namespace Lython.Runtime.Ops

open Lython.Runtime
open Lython.AST (BinOp UnaryOp CmpOp BoolOp)
...
partial def evalBoolOp (op : BoolOp) (vals : List Value)
    (evalNext : Lython.AST.Expr → InterpM Value) (exprs : List Lython.AST.Expr)
    : InterpM Value := do
  match op with
  | .and_ => do
